...
import { MapPin, BookOpen, ChevronRight, Star, Sparkles, Award, Users, CheckCircle2 } from "lucide-react";
...
export default function UniversityCard({ university }: UniversityCardProps) {
  ...
  return (
    <Card3DTilt glowColor={`${accentColor}25`} className="h-full">
      <article className="group relative flex flex-col h-full bg-white border border-[#E2E8F0] rounded-2xl overflow-hidden transition-all duration-500 hover:shadow-2xl hover:border-transparent">
        ...
        <div className="p-5 sm:p-6 flex flex-col flex-1 bg-white">
          ...
          {/* University Title */}
          <h3 className="font-bold text-slate-900 text-lg leading-snug mb-2 group-hover:text-[#0B3C5D] transition-colors line-clamp-2">
            {university.name}
          </h3>

          {/* Description */}
          <p className="text-xs sm:text-sm text-slate-600 leading-relaxed mb-4 flex-1 line-clamp-2">
            {university.description}
          </p>

          {/* Performance Stats Bar (Placement, Students, Programs) */}
          ...
          <div className="flex items-center gap-2.5 mt-auto pt-3 border-t border-slate-100">
            <motion.div className="flex-1" whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.97 }}>
              <Link
                href={`/universities`}
                className="w-full flex items-center justify-center gap-1 py-2.5 text-xs font-bold text-[#0B3C5D] border border-slate-200 rounded-xl hover:bg-[#0B3C5D] hover:text-white hover:border-[#0B3C5D] transition-all duration-200"
              >
                View Details
            ...
            <motion.div className="flex-1" whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.97 }}>
              <Link
                ...
                className="w-full flex items-center justify-center py-2.5 text-xs font-bold bg-[#F97316] text-white rounded-xl hover:bg-[#ea6c0c] transition-colors duration-200 shadow-sm"
              >
                Apply / Enquire
              </Link>
            </motion.div>
